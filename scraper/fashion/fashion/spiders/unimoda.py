...
class UnimodaSpider(scrapy.Spider):
    ...
    def __init__(self, category: str = None, starting_page: int = 1, *args, **kwargs):
        ...
        if category:
            self.start_urls = [
                f'https://unimoda.cz/{category}?page={starting_page}'
            ]
        else:
            self.start_urls = [
    ...
    def parse(self, response):
        ...
        if next_page_num < number_of_pages:
            next_page_url = response.url.replace(f"page={curr_page_num}", f"page={next_page_num}")  #f'{"".join(response.url.split("page=")[:-1])}page={next_page_num+1}'
