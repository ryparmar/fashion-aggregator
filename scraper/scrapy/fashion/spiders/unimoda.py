...
from fashion.items import UnimodaItem


class UnimodaSpider(scrapy.Spider):
    name = "unimoda"
    valid_categories = ("damske", "panske", "detske", "obuv", "doplnky", "kabelky")

    def __init__(self, category: str = None, starting_page: int = 1, *args, **kwargs):
        super(UnimodaSpider, self).__init__(*args, **kwargs)
        ...
        if category:
            self.start_urls = [f"https://unimoda.cz/{category}?page={starting_page}"]
        else:
            self.start_urls = [
                f"https://unimoda.cz/damske?page={starting_page}",
                f"https://unimoda.cz/panske?page={starting_page}"
                f"https://unimoda.cz/detske?page={starting_page}"
                f"https://unimoda.cz/obuv?page={starting_page}"
                f"https://unimoda.cz/doplnky?page={starting_page}"
                f"https://unimoda.cz/kabelky?page={starting_page}",
            ]
    ...
    def parse_item(self, response) -> UnimodaItem:
        ...
        item["sizes"] = (
            " ".join(response.css("span.size-attribute").css("span::text").getall()).replace(" | ", " ").split()
        )
